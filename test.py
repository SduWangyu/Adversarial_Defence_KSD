...
if __name__ == "__main__":
    net = ae.ConvAutoEncoderMNIST()
    trans_train = transforms.Compose([transforms.ToTensor()])
    trans_test = transforms.Compose([transforms.ToTensor()])
    train_iter, test_iter = load_data("mnist", batch_size=64, transforms_train=trans_train,
                                      transforms_test=trans_test)
    ...
    train_autoencoder(net, train_iter, test_iter, num_epochs=100, lr=0.01,
                      device=try_gpu(), model_name='./data/models_trained/autoencoders/cov_mnist.pth')
