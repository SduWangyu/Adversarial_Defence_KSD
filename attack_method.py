...
import numpy as np
from picsop import image_preprocessing
from picsop import transform_to_img_from_dataset, show_images_diff
import torchvision.models as models


def fgsm(model, img_tensor, e=0.001, max_iterations=100, target_label=288):
    """
    fgsm 目标攻击算法
    :param model: 要攻击的模型
    ...
    optimizer = torch.optim.Adam([img_tensor])
    ...
    for epoch in range(max_iterations):
        ...
        # 反向传递 计算梯度
        loss.backward()
        img_tensor.data = img_tensor.data - e * torch.sign(img_tensor.grad.data)
    else:
        return False


def deepfool(model, img_tensor, original_label, max_iterations=100, num_classes=1000, overshoot=0.02):
    img_tensor.requires_grad = True
    input_shape = img_tensor.cpu().detach().numpy().shape
    w = np.zeros(input_shape)
    r_tot = np.zeros(input_shape)
    output = model(img_tensor)
    for epoch in range(max_iterations):
        scores = model(img_tensor).data.cpu().numpy()[0]
        ...
        grad_orig = img_tensor.grad.data.cpu().numpy().copy()
        for k in range(1, num_classes):
            ...
            cur_grad = img_tensor.grad.data.cpu().numpy().copy()
        ...
        img_tensor.data = img_tensor.data + (1 + overshoot) * torch.from_numpy(r_tot).to(device)
...
def deepfool_target(model, img_tensor, target_label,
    ...
    for epoch in range(max_iterations):
        ...
        w = img_tensor.grad.data.cpu().numpy().copy()
        f = output[0, target_label].data.cpu().numpy()
        pert = abs(f) / np.linalg.norm(w.flatten())
        # 计算 r_i 和 r_tot
        r_i = (pert + 1e-8) * w / np.linalg.norm(w)
        r_tot = np.float32(r_tot + r_i)
        img_tensor.data = img_tensor.data + (1 + overshoot) * torch.Tensor(r_tot).to(device)
...
if __name__ == "__main__":
    ...
    # deepfool_target(model, adv_image_tensor, target_label=288)
    fgsm(model, adv_image_tensor)
    # deepfool(model, adv_image_tensor, original_label=label)
    # adv_image_tensor = cw(model, adv_image_tensor, target_label=288)
    transform_to_img_from_dataset(adv_image_tensor.cpu().clone().detach(), ToTensor_transform, "pics/test111_adv.png")
    show_images_diff(original_image_tensor, adv_image_tensor.cpu().clone().detach(), ToTensor_transform, label, 288)
